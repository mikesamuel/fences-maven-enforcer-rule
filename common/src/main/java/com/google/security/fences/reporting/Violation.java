package com.google.security.fences.reporting;
...
public final class Violation implements Comparable<Violation> {
   ...
   * @param artifact
   * The artifact containing the use of the sensitive API that was denied.
   * @param useSiteContainer The namespace in which the violation occurred.
   * @param useSiteSource
   *   The source of the use of the sensitive API that was denied.
   *   This is a human-readable form of useSiteContainer, and probably
   *   corresponds to a relative path of a .java file in the file-system on
   *   which artifact was compiled, but nothing obviously available to the
   *   currently running process.
   * @param useSiteLineNumber The line number of the use of the sensitive API
   *   that was banned or -1 if the user was compiled without debug symbols.
   * @param useSiteApiElement An API element on a sub-type (non-strict) of
   *   sensitiveApiElement.
   * @param sensitiveApiElement The API element to which access was denied.
   */
  public Violation(
      Artifact artifact,
